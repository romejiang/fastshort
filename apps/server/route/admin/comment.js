...
export default {
  ...
  async index(ctx) {
    ...
    try {
      ...
      const list = await mongo
        ...
        .skip(skip)
        .toArray()
      const total = await mongo.col('comment').countDocuments(query)
      success(ctx, {
        total,
        list
      })
  ...
  async get(ctx) {
    try {
      ...
      const comment = await mongo.col('comment').findOne({ _id: new ObjectId(id) })
      success(ctx, comment)
    } catch (error) {
      console.log(error)
      fail(ctx, '服务器错误')
  ...
  async delete(ctx) {
    try {
      const { id } = ctx.params
      const ret = await mongo.col('comment').deleteOne({ _id: new ObjectId(id) })
  ...
  async deleteMany(ctx) {
    ...
    try {
      ...
      const ret = await mongo.col('comment').deleteMany({ _id: { $in: ids } })
      if (ret.deleteCount === 0) {
        fail(ctx, '删除失败')
        return
      }
