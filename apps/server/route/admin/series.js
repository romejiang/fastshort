...
export default {
  ...
  async index(ctx) {
    ...
    try {
      ...
      const list = await mongo
        ...
        .skip(skip)
        .toArray()
      const total = await mongo.col('series').countDocuments(query)
      success(ctx, {
        total,
        list
      })
  ...
  async list(ctx) {
    try {
      ...
      const records = await mongo
        ...
        .skip(offset)
        .toArray()
      const total = await mongo.col('series').countDocuments(form)
      success(ctx, {
        currentPage,
        pageSize,
        total,
  ...
  async create(ctx) {
    ...
    const ret = await mongo.col('series').insertOne(document)
    try {
  ...
  async delete(ctx) {
    try {
      const { id } = ctx.params
      const ret = await mongo.col('series').deleteOne({ _id: new ObjectId(id) })
  ...
  async deleteMany(ctx) {
    ...
    try {
      ...
      const ret = await mongo.col('series').deleteMany({ _id: { $in: ids } })
      if (ret.deleteCount === 0) {
        fail(ctx, '删除失败')
        return
      }
